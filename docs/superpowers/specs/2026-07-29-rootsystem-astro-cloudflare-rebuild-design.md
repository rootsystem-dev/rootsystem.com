...
| D12 | Forms post to **D1**, not email or a third party | Contact (rootsystem.com) and case intake (forensics) write to a single `rootsystem-forms` D1 database with two tables. Chosen over an email-only handler so enquiries are durable and queryable rather than dependent on a mailbox, and over Typeform so the site owns its own funnel. Resend notification is deliberately deferred to post-launch; until then submissions are reviewed directly. |
| D13 | Sites keep `output: 'static'`, with the **Cloudflare adapter** for two routes only | A static build cannot accept a POST. Rather than moving both sites to server rendering, the adapter is added and only the form endpoints — plus the two pages that display submission state — opt out of prerendering via `export const prerender = false`. Every marketing page and blog post remains a static file at the edge. |
